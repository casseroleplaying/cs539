...
/* load: reads data from source. allocates space for individual structures as
 * needed. assigns addresses of structures to pointer array, and relevant data
 * to members. records number of structures created */
int load(DP *info, int *n)
{
        char buffer[81];
        
        while (fgets(buffer, 81, stdin) != NULL && *n < MAX) {
...
/* write: output contents of structures held in pointer array */
void write(DP *info, int n)
{      
        for (int i = 0; i < n; i++) {
                printf("%s" "%s" "%s" "%d\n", info[i]->name, info[i]->add, 
                       info[i]->cst, info[i]->zip);
        }
}
